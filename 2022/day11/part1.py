...
def generate_op(a, b, operator):
    ...
    return lambda x: m_operator(a if a != 'old' else x, b if b != 'old' else x) // 3
...
for _ in range(20):
    ...
    for i in range(len(monkeys)):
        ...
        for j in range(len(monkey.items)):
            ...
            print(monkey.items[j])
            print(monkey.op(79))
            monkey.items[j] = monkey.op(monkey.items[j])
            print(monkey.items[j])
            
            target = monkey.test(monkey.items[j])
            print(f'monkey {i} throw to {target}')
